...
const metaSubscribers = new Set<
  (playerId: string, meta: PlayerMetaInfo) => void
>();
...
export function onPlayerMeta(
  cb: (playerId: string, meta: PlayerMetaInfo) => void
) {
...
export function createPlayerRealtime(options: {
  roomId: string
  me: PlayerMeta // includes userId, name, character
  handlers?: {
  ...
  return {
    broadcastPosition: (data: { x: number; y: number }) =>
